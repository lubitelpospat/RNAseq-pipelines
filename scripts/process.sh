...
if (( $( ls -l /root | grep SRR  - | wc -l)  == 2)); then
hisat2 -p 4 -x /root/genome/grch38/genome -1 $SRA\_1.fastq.gz -2 $SRA\_2.fastq.gz -S /root/$SRA.sam > /root/data/$SRA/log/hisat2.log 2>&1
else
    hisat2 -p 4 -x /root/genome/grch38/genome -U $SRA.fastq.gz -S /root/$SRA.sam > /root/data/$SRA/log/hisat2.log 2>&1
fi
